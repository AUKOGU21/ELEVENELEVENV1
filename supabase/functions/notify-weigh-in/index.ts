...
// -----------------------------------------------------------------------------
// Emails the owner of a decision when someone weighs in on it. Called by the
// app (Feed.tsx) right after a row is inserted into `responses`, with
// { decision_id, responder_id }. Everything that matters (owner email,
// responder name, item) is re-resolved server-side with the service role, so
// the client is only trusted to name which decision was responded to.
//
// Secrets (reused from the welcome pipeline, nothing new to set):
...
//
// Source of truth for the design is emails/weigh-in.html — keep TEMPLATE in sync.
// -----------------------------------------------------------------------------
...
// Keep in sync with emails/weigh-in.html
...
<body style="margin:0;padding:0;background:#ffffff;">
  <div style="display:none;max-height:0;overflow:hidden;mso-hide:all;opacity:0;color:transparent;height:0;width:0;">
    {{RESPONDER}} just weighed in on {{ITEM}}.
  </div>
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" class="container" style="width:600px;max-width:600px;background:#ffffff;">
          ...
          <tr>
            <td align="center" class="px" style="padding:26px 56px 8px;font-size:17px;line-height:1.5;color:#3A3530;">
              <strong style="font-weight:600;color:#100E0C;">{{RESPONDER}}</strong> just shared their take on {{ITEM}}.
            </td>
          </tr>
          <tr>
            <td align="center" class="px" style="padding:0 56px 28px;font-size:15px;line-height:1.6;color:#6F665A;">
              see what they said before you decide.
            </td>
          </tr>
          <tr>
            <td align="center" style="padding:6px 48px 4px;">
              ...
              <v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word" href="{{SITE_URL}}/feed" style="height:48px;v-text-anchor:middle;width:240px;" arcsize="0%" stroke="f" fillcolor="#CB5A3C">
                <w:anchorlock/>
                <center style="color:#ffffff;font-family:Helvetica,Arial,sans-serif;font-size:13px;letter-spacing:2px;">SEE THE WEIGH-IN</center>
              </v:roundrect>
              <![endif]-->
              <!--[if !mso]><!-- -->
          ...
          <tr>
            <td align="center" class="px" style="padding:0 48px 36px;font-size:11px;line-height:1.7;color:#b3ab9e;">
              you're getting this because someone weighed in on your decision.<br>
...
Deno.serve(async (req) => {
  ...
  const responderId: string | undefined = payload.responder_id;
  if (!decisionId) return json({ error: "decision_id required" }, 400);

  // Resolve the decision and its owner.
  ...
  const ownerId: string = decision.user_id;
  // Never notify someone about their own weigh-in.
  if (responderId && ownerId === responderId) return json({ skipped: "self weigh-in" });

  // Owner's email (auth) + responder's display name (profiles).
  ...
  if (!ownerEmail) return json({ skipped: "owner email not found" });

  let responderName = "someone";
  if (responderId) {
    const profs = await sb(`profiles?id=eq.${responderId}&select=display_name`);
    responderName = firstName(Array.isArray(profs) ? profs[0]?.display_name : null);
  }
  ...
  const html = TEMPLATE
    .replace(/\{\{RESPONDER\}\}/g, esc(responderName))
    .replace(/\{\{ITEM\}\}/g, esc(item))
    .replace(/\{\{SITE_URL\}\}/g, SITE_URL);

  const subject = item === "your pick"
    ? `${responderName} weighed in on your pick`
    : `${responderName} weighed in on your ${item}`;
  ...
  console.log("Weigh-in email sent to", ownerEmail, data?.id);
