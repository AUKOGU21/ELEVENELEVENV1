    ...
    <div style={{ background: "rgba(0,0,0,0.035)", borderRadius: 14, padding: "13px 15px", border: "1px solid rgba(0,0,0,0.06)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8, gap: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
          ...
          <div style={{ minWidth: 0, display: "flex", alignItems: "center", flexWrap: "wrap" }}>
            <span style={{ fontSize: 14.5, fontWeight: 700, color: "#1A1A1A" }}>{formatName(rec.profiles?.display_name ?? null)}</span>
            <MatchBadge score={rec.match_score} />
          </div>
        </div>
        <div style={{ flexShrink: 0, borderRadius: 100, padding: "3px 10px", fontSize: 12.5, fontWeight: 600, whiteSpace: "nowrap", background: isBuy ? "rgba(22,163,74,0.10)" : "rgba(192,57,43,0.10)", color: isBuy ? "#16a34a" : "#c0392b" }}>
          {recommendationLabel(rec.recommendation)}
        </div>
      </div>

      {/* Reasoning */}
      <p style={{ fontSize: 14.5, lineHeight: 1.55, color: "#5A4A42", margin: "0 0 10px" }}>{rec.reasoning}</p>
      ...
      {(rec.product_name || rec.brand_name || rec.product_image_url || rec.product_url) && (
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {rec.product_name && <p style={{ fontSize: 12.5, color: MUTED, margin: "1px 0 0", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{rec.product_name}</p>}
            {price && <p style={{ fontSize: 13.5, fontWeight: 600, color: INK, margin: "3px 0 0" }}>{price}</p>}
          </div>
      ...
      {(rec.fit_note || rec.who_for) && (
        <div style={{ marginBottom: 12, display: "flex", flexDirection: "column", gap: 4 }}>
          {rec.fit_note && <p style={{ fontSize: 13, color: "#5A4A42", margin: 0 }}><span style={{ color: MUTED }}>Fit —</span> {rec.fit_note}</p>}
          {rec.who_for && <p style={{ fontSize: 13, color: "#5A4A42", margin: 0 }}><span style={{ color: MUTED }}>Best for —</span> {rec.who_for}</p>}
        </div>
      )}
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
        ...
        <span style={{ fontSize: 13, color: MUTED }}>{timeAgo(rec.created_at)}</span>
      </div>
    </div>
  );
}
