...
def send(email, first):
    html = template.replace("{{FIRST_NAME}}", first).replace("{{UNSUBSCRIBE_URL}}", UNSUB)
...
if DRY:
    for r in people:
        print("  ", r["email"], "-", r["first"] or "(no name)")
    sys.exit(0)

sent = 0
for r in people:
    mid, raw = send(r["email"], r["first"] or "there")
    if mid:
